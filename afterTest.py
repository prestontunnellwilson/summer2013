#current as of 8/7/13, 4:26
#most recent changes:
#uses ankle instead of foot
#writes the angle difference to the file
#makes a file based on subject initial
#has delimeters in output to file
#switches normal trackers (trying)
...
test = True
sight = test  				#whether objects and scenery dissapear. Useful for testing
validID = False
while not (validID):
	try:
...
#if not testing, get initial of subject
if test:
	subjectInitial = 'test'
else:
	subjectInitial = (viz.input('Input subject inital'))
	
#Determine whether or not trial recording is enabled. We don't need to record if testing. can set to option2 (see hopelean)
recordingEnabled = not test

#if recording, make directory and file for recording
if recordingEnabled:
...
#always debugging? at least for now.	
...
HEIGHT_TO_STRIDE = .414			#not used. can search online for this conversion
strideLength = 1
print "strideLength = %f" %strideLength

#add headmounted display tracker
...
#initialize the trackers for both kinects
for i in range (24):
...
viz.link(tracker, view)				#LINK headmounted display tracker with view

#########################step stuff########################
foot = False		#false left foot, true right foot, mostly used for debugging
aIsTrue = False		#whether we are using KinectA or not

###################gaze angle stuff#######################
finalYaw = 0		#curren yaw
previousYaw = 0		#previous yaw, used for turning
turning = 0			#flag if we are moving. prohibits movement if so
stepCount = 0		#not really used anymore...

########################foot angle stuff#####################
aveFootAngle1 = 0	#average angle value of one leg
aveFootAngle2 = 0	#average angle value of the other leg
footAngle1 = []		#array to store angle values
footAngle2 = []		#array to store the other leg's angle values
...
#I am not sure what objectSet does... I think it was used for move and rotate which we don't use
objectSet = 0
...
def checkTurning():
	...
	if turning and wait:
		...
		#now accounts for circle
		turning = ((finalYaw < (previousYaw - angleThreshold) %360 ) or (finalYaw > (previousYaw + angleThreshold) % 360))
...
ERROR = 43- off	#AMOUNT OF DEGREES to correct movement
def step():
	...
	stepCount += 1
	wantToKnowStepCount = False
	if wantToKnowStepCount:
...
def checkStep():
	...
	stepInfo = True
	bigAngle = 165
	#if we were moving and shouldn't anymore
	if moving and ((aveFootAngle1 >= bigAngle and aveFootAngle2 >= bigAngle) or turning):
		moving = False
		view.velocity(0,0,0)
	#if we are not already moving, check to see if we should
...
def move(scale = 1):
	...
	alpha = data[3]  - ERROR		#subtracting 45 degrees seems to correct direction, no need
										#to mod 360 since we use trig functions
...
def changeState(state):
	...
	#walk toward the target and face the right direction
		...
		cylinder.setEuler(targetRotation)		#why do we need this? Cylinder doesn't need to rotate
...
def ShowMessage(mystring):
	...
	messagePos = (.5,.5)		#change this if you want the message to be in a differen position
...
def keyEvent(key):
	...
	global startPosition, startAngle, targetPosition, height, recordingEnabled, recordFile
	...
	#this button includes many of the other buttons, making switchin easier
	elif(key == 'a'):
		changeConditions()
	...
	#update the state
		...
		global pause
		pause = not pause
	#makes subject sink. Not actually used, implemented in a test program. Its counterpart key press f
	# disappeared...
	elif key == 'd':
		...
		redoTrial(int(key))
	#else any other key was pressed, just print out where person is
	else:
		print view.getPosition()
...
def update():
	...
	if(currentState == 1 or currentState == 3):
		...
		if(distSqrd <= radius):
			cylinder.color(0,1,0)			#make cylinder green
			...
			#else not close enough to either and both should be red
			cylinder.color(1,0,0)
			arrow.color(1,0,0)

	#elif could cause some interesting problems, so we will just say if as of 8/7/2013. But example
	# should be cleared before real objects are shown. //push 'e' before ' '
	if example:
...
#initialize arrays and locations and everything!
FileReader()		
...
if debugInitialize:
	print targetLocations
	print targetObjects
#put up example object and cylinder
example = False
keyEvent('e')
#rotate once
#keyEvent('r')	
view.setEuler(45,0,0)	#silly, because view and tracker are linked

#things to do regularly. A repeat value of $0$ means as often as possible
